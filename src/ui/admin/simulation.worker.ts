...
self.onmessage = (event: MessageEvent<SimulationWorkerRequest>) => {
  ...
  try {
    ...
    if (!templateResult.ok) {
      const response: SimulationWorkerResponse = { id: message.id, workerIndex: message.workerIndex, ok: false, error: templateResult.error };
      self.postMessage(response);
      return;
    }
    const ranksResult = importSharedRanksJson(message.ranksJson);
    if (!ranksResult.ok) {
      const response: SimulationWorkerResponse = { id: message.id, workerIndex: message.workerIndex, ok: false, error: ranksResult.error };
      self.postMessage(response);
      return;
    }
    ...
    const response: SimulationWorkerResponse = { id: message.id, workerIndex: message.workerIndex, ok: true, aggregate };
    self.postMessage(response);
  } catch (error) {
