...
export const AdminRanksTab = ({
  t, lang, exportRanksToFile, importRanks, importRanksFromFile, ranksImportError, ranksImportStatus,
  ...
  t: T; lang: 'uk' | 'en'; exportRanksToFile: () => void; importRanks: () => void; importRanksFromFile: (file: File | null) => void;
  ...
  <>
    ...
    <div className="admin-deck-list"><ul>
      {editableRanks.map((rank, index) => (
        <li key={`rank-${rank.id}-${index}`}><div className="admin-inline-editor"><div className="admin-editor-grid">
          ...
          <label>{lang === 'uk' ? 'Назва' : 'Name'}<input value={rank.name} onChange={(e) => updateRankAt(index, (row) => ({ ...row, name: e.target.value }))} /></label>
          <label>{lang === 'uk' ? 'Зображення' : 'Image'}<input value={rank.image ?? ''} onChange={(e) => updateRankAt(index, (row) => ({ ...row, image: e.target.value }))} placeholder="/cards/rank-*.webp" /></label>
          <label>{lang === 'uk' ? 'Файл зображення' : 'Image file'}<input type="file" accept="image/*" onChange={(e) => { void attachRankImageFile(index, rank.id, e.target.files?.[0] ?? null); e.currentTarget.value = ''; }} /></label>
    ...
    <div className="admin-inline-editor"><div className="admin-editor-grid">
      ...
      <label>{lang === 'uk' ? 'Назва' : 'Name'}<input value={rankDraft.name} onChange={(e) => setRankDraft((prev: any) => ({ ...prev, name: e.target.value }))} /></label>
      <label>{lang === 'uk' ? 'Зображення' : 'Image'}<input value={rankDraft.image ?? ''} onChange={(e) => setRankDraft((prev: any) => ({ ...prev, image: e.target.value }))} placeholder="/cards/rank-*.webp" /></label>
      <label>{lang === 'uk' ? 'Файл зображення' : 'Image file'}<input type="file" accept="image/*" onChange={(e) => { void attachRankDraftImageFile(e.target.files?.[0] ?? null); e.currentTarget.value = ''; }} /></label>
