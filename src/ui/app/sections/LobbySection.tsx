...
export const LobbySection = ({
  ...
  const [roomFilter, setRoomFilter] = useState<
    'all' | 'open' | 'free' | 'no_bots' | 'standard' | 'standard_plus'
  >('all');
  ...
  const toggleModule = (id: string, alwaysOn: boolean) => {
    ...
    if (selectedOptionalModuleIds.includes(id)) {
      setSelectedOptionalModuleIds(
        selectedOptionalModuleIds.filter((row) => row !== id),
      );
      return;
    }
  ...
  const effectivePlayerName =
    playerName.trim() || fallbackPlayerName?.trim() || '';
  const invitedRoomId = useMemo(
  ...
  const visibleMatches = useMemo(() => {
    const filtered = matches.filter((match) => {
      ...
      const taken = match.players.filter((player) =>
        Boolean(player.name),
      ).length;
      ...
      const hasBots =
        Math.max(0, Math.floor(match.setupData?.bots?.count ?? 0)) > 0;
      ...
      if (roomFilter === 'standard_plus')
        return gameModeValue === 'standard_plus';
      return true;
    });
    return filtered.sort((a, b) => {
      ...
      const aAlmostReady =
        aTaken < a.players.length && aTaken + 1 === a.players.length ? 1 : 0;
      const bAlmostReady =
        bTaken < b.players.length && bTaken + 1 === b.players.length ? 1 : 0;
  ...
  if (session && !canStart) {
    const missingSeats = activeSessionMatch
      ? activeSessionMatch.players.filter((player) => !player.name?.trim())
          .length
      : 0;
    return (
      <section
        className={`board board-v2-panel board-v2-active-room${uiVariant === 'v1' ? ' board-v1-panel board-v1-active-room' : ''}`}
      >
        <h2 className="lobby-active-room-title">
          {t.activeRoom}: {session.matchID}
        </h2>
        <p className="lobby-active-room-meta">
          {t.joinedAs}: {effectivePlayerName || '-'} (#{session.playerID})
        </p>
        <div className="lobby-room-blockers">
          <p>
            {activeSessionMatch
              ? t.roomBlockedNeedPlayersCount.replace(
                  '{count}',
                  String(missingSeats),
                )
              : t.loadingRooms}
        ...
        {activeSessionMatch ? (
          <div className="lobby-room-seat-list">
            {activeSessionMatch.players.map((player) => (
              <span
                key={`${activeSessionMatch.matchID}-waiting-seat-${player.id}`}
                className={`lobby-room-seat${player.name ? ' is-filled' : ' is-empty'}`}
              >
  ...
  return (
    <section
      className={`board board-v2-panel board-v2-lobby${uiVariant === 'v1' ? ' board-v1-panel board-v1-lobby' : ''}`}
    >
      <h2>{t.lobbyTitle}</h2>
        ...
        <div className="lobby-col board-v2-column">
          ...
          <div className="lobby-room-filters">
            ...
            <button
              ...
              disabled={roomFilter === 'open'}
            >
              {t.roomStatusOpen}
            </button>
            <button
              type="button"
              onClick={() => setRoomFilter('free')}
              disabled={roomFilter === 'free'}
            >
              {t.lobbyAlmostReady}
            </button>
            <button
              type="button"
              onClick={() => setRoomFilter('no_bots')}
              disabled={roomFilter === 'no_bots'}
            >
              {t.lobbyFilterNoBots}
            </button>
            <button
              type="button"
              onClick={() => setRoomFilter('standard')}
              disabled={roomFilter === 'standard'}
            >
              {t.gameModeStandard}
            </button>
            <button
              type="button"
              onClick={() => setRoomFilter('standard_plus')}
              disabled={roomFilter === 'standard_plus'}
            >
          ...
          <div className="lobby-room-list">
            {visibleMatches.map((match) => {
              ...
              const botCountValue = Math.max(
                0,
                Math.min(capacity - 1, Math.floor(botSetup?.count ?? 0)),
              );
              const legendMode =
                match.setupData?.gameSetup?.legendaryDeckMode ?? 'separate';
              const optionalModules =
                match.setupData?.gameSetup?.optionalMainDeckModuleIds ?? [];
              ...
              return (
                <article
                  key={match.matchID}
                  className={`lobby-room-card${almostReady ? ' is-almost-ready' : ''}${invitedRoomId === match.matchID ? ' is-invited' : ''}`}
                >
                  <div className="lobby-room-card-head">
                    <div>
                      <strong>{match.matchID}</strong>
                      <p>
                        {formatGameModeLabel(t, gameModeValue)} · {taken}/
                        {capacity} ·{' '}
                        {hasFree
                          ? almostReady
                            ? t.lobbyAlmostReady
                    ...
                    <span
                      className={`pill pill-badge${almostReady ? ' pill-badge-good' : ''}`}
                    >
                      {almostReady
                        ? t.lobbyAlmostReady
                        : hasFree
                  ...
                  {invitedRoomId === match.matchID ? (
                    <p className="lobby-room-invite-badge">
                      {t.roomInviteLinkHint}
                    </p>
                  ) : null}
                  <div className="lobby-room-summary-grid">
                    <span>{t.roomSummaryPlayers}</span>
                    <strong>
                      {taken}/{capacity}
                    </strong>
                    <span>{t.gameModeLabel}</span>
                    <strong>{formatGameModeLabel(t, gameModeValue)}</strong>
                    <span>{t.roomBotsLabel}</span>
                    <strong>
                      {botCountValue > 0
                        ? `${botCountValue} · ${formatBotDifficultyLabel(t, botSetup?.difficulty ?? null)}`
                        : t.roomBotsOff}
                    </strong>
                    <span>{t.legendaryModeLabel}</span>
                    <strong>
                      {legendMode === 'merged'
                        ? t.legendaryModeMerged
                        : t.legendaryModeSeparate}
                    </strong>
                    <span>{t.roomModulesLabel}</span>
                    <strong>
                      {formatModuleList(optionalModules, moduleNameById)}
                    </strong>
                    <span>{t.roomSummarySeat}</span>
                    <strong>{freePlayer ? `#${freePlayer.id}` : '-'}</strong>
                  </div>
                  <div className="lobby-room-seat-list">
                    {match.players.map((player) => (
                      <span
                        key={`${match.matchID}-seat-${player.id}`}
                        className={`lobby-room-seat${player.name ? ' is-filled' : ' is-empty'}`}
                      >
                        #{player.id} {player.name?.trim() || t.lobbySeatOpen}
                      </span>
        ...
        <div className="lobby-col board-v2-column">
          ...
          <p className="admin-controls">
            {allowedRoomCapacities.map((size) => (
              <button
                key={`room-cap-${size}`}
                type="button"
                aria-pressed={roomCapacity === size}
                onClick={() => setRoomCapacity(size)}
              >
                {roomCapacity === size ? '✓ ' : ''}
                {size}
              </button>
          ...
          <p className="admin-controls">
            {optionalModules.map((module) => {
              const enabled =
                selectedOptionalModuleIds.includes(module.id) ||
                module.alwaysOn;
              return (
                <button
                ...
                >
                  {enabled ? '✓ ' : ''}
                  {module.name}
                  {module.alwaysOn ? ` (${t.roomModuleAlwaysOn})` : ''}
                </button>
              );
            })}
          </p>
          ...
          <p className="admin-controls">
            <button
              ...
              onClick={() => setCreateWithBots(false)}
            >
              {!createWithBots ? '✓ ' : ''}
              {t.roomBotsOff}
            </button>
              ...
              <p className="admin-controls">
                {[
                  ...
                  <button
                    ...
                    onClick={() =>
                      setBotDifficulty(difficulty.id as BotDifficulty)
                    }
                  >
          ...
          <p className="admin-controls">
            <button
              type="button"
              onClick={createRoom}
              disabled={!effectivePlayerName || loading}
            >
              {t.createRoom}
            </button>
          </p>
          <div
            className={`lobby-room-create-summary lobby-room-create-summary-v4-compact board-v2-subpanel${uiVariant === 'v1' ? ' board-v1-subpanel' : ''}`}
          >
            <h4>{t.roomSummaryReady}</h4>
            <ul>
              <li>
                {t.gameModeLabel}: {formatGameModeLabel(t, gameMode)}
              </li>
              <li>
                {t.roomCapacity}: {roomCapacity}
              </li>
              <li>
                {t.roomBotsLabel}:{' '}
                {createWithBots
                  ? `${effectiveBotCount} · ${formatBotDifficultyLabel(t, botDifficulty)}`
                  : t.roomBotsOff}
              </li>
              <li>
                {t.roomModulesLabel}:{' '}
                {formatModuleList(selectedOptionalModuleIds, moduleNameById)}
              </li>
              <li>
                {t.roomDurationLabel}:{' '}
                {estimateRoomDurationLabel(t, roomCapacity, gameMode)}
              </li>
            </ul>
            <p>{t.roomDraftHint}</p>
          </div>
