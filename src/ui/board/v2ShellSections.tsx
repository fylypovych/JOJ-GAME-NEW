export const V2EndVoteModal = (props: {
  ...
  return (
    <section className="game-ui-v2-vote-popup game-ui-layout-vote-popup" role="dialog" aria-label={props.title}>
...
export const V2StandingsSummary = (props: {
  ...
  playerSummaries: Array<{
    ...
    <div className="game-ui-v2-gameover-summary game-ui-layout-gameover-summary">
    <h4>{props.title}</h4>
      <div className="game-ui-v2-gameover-summary-list game-ui-layout-gameover-summary-list">
      {props.playerSummaries.map((row) => (
        <article key={`summary-${row.playerID}`} className={`game-ui-v2-gameover-summary-row game-ui-layout-gameover-summary-row${row.winner ? ' is-winner' : ''}`}>
          <div className="game-ui-v2-gameover-summary-head game-ui-layout-gameover-summary-head">
            <strong>{row.name}</strong>
            <span>{row.rankName}</span>
          </div>
          <div className="game-ui-v2-gameover-summary-grid game-ui-layout-gameover-summary-grid">
...
export const V2GameoverModal = (props: {
  ...
  if (!props.open) return null;
  return (
    <div className="game-ui-v2-gameover-modal game-ui-layout-gameover-modal" role="dialog" aria-label={props.ariaLabel}>
      <div className="game-ui-v2-gameover-card game-ui-layout-gameover-card">
        <h3>{props.title}</h3>
        <p>
        ...
        <div className="game-ui-v2-token-list game-ui-layout-token-list">
          <div className="game-ui-v2-token-row game-ui-layout-token-row"><span>{props.statsLabels.totalTurns}</span><strong>{props.stats.totalTurns}</strong></div>
          <div className="game-ui-v2-token-row game-ui-layout-token-row"><span>{props.statsLabels.resourcesGained}</span><strong>{props.stats.resourcesGained}</strong></div>
          <div className="game-ui-v2-token-row game-ui-layout-token-row"><span>{props.statsLabels.resourcesLost}</span><strong>{props.stats.resourcesLost}</strong></div>
          <div className="game-ui-v2-token-row game-ui-layout-token-row"><span>{props.statsLabels.lyapsPlayed}</span><strong>{props.stats.lyapsPlayed}</strong></div>
          <div className="game-ui-v2-token-row game-ui-layout-token-row"><span>{props.statsLabels.scandalsPlayed}</span><strong>{props.stats.scandalsPlayed}</strong></div>
        </div>
        <V2StandingsSummary
