...
const text = (value: unknown, max = 50_000) =>
  String(value ?? '')
    .trim()
    .slice(0, max);
...
const safeSlug = (value: unknown) =>
  safeId(value).toLowerCase() || `news-${Date.now()}`;

const emptyMaterials = (): DownloadMaterialsDocument => ({
...
const readMaterials = async (
  filePath: string,
): Promise<DownloadMaterialsDocument> => {
  try {
    const parsed = JSON.parse(
      await readFile(filePath, 'utf8'),
    ) as Partial<DownloadMaterialsDocument>;
    return {
      kind: 'joj-download-materials',
      version: 1,
...
const writeMaterials = async (
  filePath: string,
  document: DownloadMaterialsDocument,
) => {
...
const mapPage = (row: Record<string, unknown>): ProjectPage => ({
  key: 'rules',
  title: String(row.title ?? ''),
  titleEn: String(row.title_en ?? ''),
  summary: String(row.summary ?? ''),
...
const parseDataUrl = (value: unknown, allowed: Record<string, string>) => {
  ...
  if (!match || !allowed[match[1].toLowerCase()])
    throw new Error('Unsupported or invalid file');
  const buffer = Buffer.from(match[2], 'base64');
  if (!buffer.length) throw new Error('Empty file');
  return {
    buffer,
    mimeType: match[1].toLowerCase(),
    extension: allowed[match[1].toLowerCase()],
  };
};
...
const saveUpload = async (
  ...
  while (
    path.basename(publicDir) !== 'public' &&
    path.dirname(publicDir) !== publicDir
  )
    publicDir = path.dirname(publicDir);
  if (path.basename(publicDir) !== 'public')
    throw new Error('Upload directory must be inside public');
...
const removeUpload = async (
  ...
  const appRoot = path.dirname(publicDir);
  await unlink(
    path.join(appRoot, 'dist', path.basename(dir), ...relative.split('/')),
  ).catch(() => undefined);
};
...
export const registerContentRoutes = (deps: ContentRouteDeps) => {
  ...
  router.get('/api/content/news', async (ctx) => {
    if (!pool) {
      ctx.status = 503;
      ctx.body = { ok: false, error: 'Database unavailable' };
      return;
    }
    const result = await pool.query(
      `${newsSelect} WHERE status = 'published' ORDER BY pinned DESC, sort_order ASC, published_at DESC NULLS LAST, updated_at DESC`,
    );
  ...
  router.get('/api/content/rules', async (ctx) => {
    if (!pool) {
      ctx.status = 503;
      ctx.body = { ok: false, error: 'Database unavailable' };
      return;
    ...
    const result =
      await pool.query(`SELECT page_key, title, title_en, summary, summary_en, body, body_en, status, updated_at
      FROM project_pages WHERE page_key = 'rules' AND status = 'published' LIMIT 1`);
    ctx.body = {
      ok: true,
      page: result.rows[0] ? mapPage(result.rows[0]) : null,
    };
  });
  ...
  router.get('/api/admin/content/news', async (ctx) => {
    ...
    const result = await pool.query(
      `${newsSelect} ORDER BY pinned DESC, sort_order ASC, updated_at DESC`,
    );
    ctx.body = { ok: true, news: result.rows.map(mapNews) };
  });

  router.get('/api/admin/content/rules', async (ctx) => {
    if (!(await deps.requireAdminAuth(ctx, 'admin-content-rules-list'))) return;
    if (!pool) {
      ctx.status = 503;
      ctx.body = { ok: false, error: 'Database unavailable' };
      return;
    ...
    const result =
      await pool.query(`SELECT page_key, title, title_en, summary, summary_en, body, body_en, status, updated_at
      FROM project_pages WHERE page_key = 'rules' LIMIT 1`);
    ctx.body = {
      ok: true,
      page: result.rows[0] ? mapPage(result.rows[0]) : null,
    };
  ...
  router.post('/api/admin/content/rules/save', async (ctx) => {
    if (!(await requireWrite(ctx, 'admin-content-rules-save'))) return;
    if (
      !(await deps.enforceRateLimit(
        ctx,
        'admin-content-rules-save',
        60,
        60_000,
      ))
    )
      return;
    const body = await deps.readJsonBodySafe({
      ctx,
      routeLabel: 'admin-content-rules-save',
      maxBytes: deps.jsonBodyLimit,
    ...
    if (!title) {
      ctx.status = 400;
      ctx.body = { ok: false, error: 'Title is required' };
      return;
    }
    const status = body.status === 'published' ? 'published' : 'draft';
    const result = await pool.query(
      `INSERT INTO project_pages
      (page_key, title, title_en, summary, summary_en, body, body_en, status)
      VALUES ('rules',$1,$2,$3,$4,$5,$6,$7)
      ON CONFLICT (page_key) DO UPDATE SET title=EXCLUDED.title,title_en=EXCLUDED.title_en,
      summary=EXCLUDED.summary,summary_en=EXCLUDED.summary_en,body=EXCLUDED.body,
      body_en=EXCLUDED.body_en,status=EXCLUDED.status,updated_at=now()
      RETURNING *`,
      [
        title,
        text(body.titleEn, 300),
        text(body.summary),
        text(body.summaryEn),
        text(body.body),
        text(body.bodyEn),
    ...
    ctx.body = { ok: true, page: mapPage(result.rows[0]) };
  });

  router.post('/api/admin/content/news/save', async (ctx) => {
    if (!(await requireWrite(ctx, 'admin-content-news-save'))) return;
    if (
      !(await deps.enforceRateLimit(ctx, 'admin-content-news-save', 60, 60_000))
    )
      return;
    const body = await deps.readJsonBodySafe({
      ctx,
      routeLabel: 'admin-content-news-save',
      maxBytes: deps.jsonBodyLimit,
      logLine: deps.logLine,
    });
    if (!body || !pool) return;
  ...
  router.post('/api/admin/content/news/delete', async (ctx) => {
    ...
    const body = await deps.readJsonBodySafe({
      ctx,
      routeLabel: 'admin-content-news-delete',
      maxBytes: deps.jsonBodyLimit,
      logLine: deps.logLine,
    });
    if (!body || !pool) return;
    const deleted = await pool.query(
      'DELETE FROM project_news WHERE id=$1::uuid RETURNING cover_image_path',
      [text(body.id, 100)],
    );
  ...
  router.post('/api/admin/content/news/image-upload', async (ctx) => {
    ...
    const body = await deps.readJsonBodySafe({
      ctx,
      routeLabel: 'admin-content-news-image',
      maxBytes: deps.uploadBodyLimit,
      logLine: deps.logLine,
    });
    if (!body) return;
    try {
      ctx.body = {
        ok: true,
        ...(await saveUpload(deps.newsAssetsDir, '/news-assets', body, {
          'image/png': 'png',
          'image/jpeg': 'jpg',
          'image/webp': 'webp',
        })),
  ...
  router.get('/api/admin/content/downloads', async (ctx) => {
    if (!(await deps.requireAdminAuth(ctx, 'admin-content-downloads-list')))
      return;
    ctx.body = {
      ok: true,
      materials: (await readMaterials(deps.materialsConfigPath)).materials,
    };
  });
  ...
  router.post('/api/admin/content/downloads/upload', async (ctx) => {
    ...
    const body = await deps.readJsonBodySafe({
      ctx,
      routeLabel: 'admin-content-download-upload',
      maxBytes: deps.uploadBodyLimit,
      logLine: deps.logLine,
    });
    if (!body) return;
    try {
      ctx.body = {
        ok: true,
        ...(await saveUpload(deps.downloadsDir, '/downloads', body, {
          'application/pdf': 'pdf',
          'application/zip': 'zip',
          'image/png': 'png',
          'image/jpeg': 'jpg',
  ...
  router.post('/api/admin/content/downloads/cover-upload', async (ctx) => {
    if (!(await requireWrite(ctx, 'admin-content-download-cover-upload')))
      return;
    const body = await deps.readJsonBodySafe({
      ctx,
      routeLabel: 'admin-content-download-cover-upload',
      maxBytes: deps.uploadBodyLimit,
      logLine: deps.logLine,
    });
    if (!body) return;
    try {
      ctx.body = {
        ok: true,
        ...(await saveUpload(
          path.join(deps.downloadsDir, 'covers'),
          '/downloads/covers',
          body,
  ...
  router.post('/api/admin/content/downloads/save', async (ctx) => {
    ...
    const body = await deps.readJsonBodySafe({
      ctx,
      routeLabel: 'admin-content-download-save',
      maxBytes: deps.jsonBodyLimit,
      logLine: deps.logLine,
    });
    if (!body) return;
    ...
    const item: DownloadMaterial = {
      id: safeId(body.id) || randomUUID(),
      title: text(body.title, 300),
      titleEn: text(body.titleEn, 300),
      description: text(body.description),
      descriptionEn: text(body.descriptionEn),
      category: text(body.category, 100),
      version: text(body.version, 60),
      filePath: text(body.filePath, 500),
      fileName: text(body.fileName, 200),
      mimeType: text(body.mimeType, 100),
      sizeBytes: Math.max(0, int(body.sizeBytes)),
      coverImagePath: text(body.coverImagePath, 500),
      published: bool(body.published),
      sortOrder: int(body.sortOrder),
      updatedAt: now,
    };
    if (!item.title || !item.filePath.startsWith('/downloads/')) {
      ctx.status = 400;
      ctx.body = { ok: false, error: 'Title and uploaded file are required' };
      return;
    }
    const index = document.materials.findIndex((row) => row.id === item.id);
    if (index >= 0) document.materials[index] = item;
    else document.materials.push(item);
  ...
  router.post('/api/admin/content/downloads/delete', async (ctx) => {
    ...
    const body = await deps.readJsonBodySafe({
      ctx,
      routeLabel: 'admin-content-download-delete',
      maxBytes: deps.jsonBodyLimit,
      logLine: deps.logLine,
    });
    if (!body) return;
    ...
    document.materials = document.materials.filter(
      (row) => row.id !== safeId(body.id),
    );
    await writeMaterials(deps.materialsConfigPath, document);
    if (item) {
