...
Релізні коміти мають містити номер версії та короткий опис змін у першому рядку:

```text
0.0.3.98 — Покращено редактор карт і навігацію між модулями
```

Номер на початку зберігає сумісність із production-оновленням, а текст після тире видно у списку комітів GitHub. Опис має бути коротким, конкретним і пояснювати результат для користувача.
